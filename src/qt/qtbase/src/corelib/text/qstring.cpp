...
#  if defined(Q_OS_WIN)
    // XXXih: wincompat: CompareStringEx and locale names are not available pre-Vista.
    #if 0
    ...
    #else
    int res = CompareStringW(LOCALE_USER_DEFAULT, 0, (LPWSTR)lhs.constData(), lhs.length(), (LPWSTR)rhs.constData(), rhs.length());
    #endif
